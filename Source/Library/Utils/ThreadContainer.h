    ...
    class ThreadContainer {
        private:
            ...
            void threadRuntime() {
                ...
                pid = getpid();
                SYSTEM(getTag(), "Temp",
                    stringbuilder() << "Thread Name Set: { " << threadName << " } PID: { " << pid << " }");
                try {
        ...
        public:
            ThreadContainer(std::shared_ptr<Controller> shared_c) {
                controller = shared_c;
            }
            ~ThreadContainer() {}
            int startThread() {
            ...
            const std::string getTag() {
                return stringbuilder() << TAG << '.' << controller->getTag(); 
            }
            int getStatusCode () {
                return statusCode;
            }
            bool getAutoRestartFlag () {
                return controller->getAutoRestartFlag();
            }
            void setDeadThreadStatusCode() {
                statusCode = -1;
            }
    };
}
