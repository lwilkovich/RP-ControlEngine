        ...
        public:
            ...
            int startThread() {
                ...
                for (unsigned int i = 0; i < threadContainerPool.size(); i++) {
                    INFO(getTag(), getDesc(), 
                        stringbuilder() << threadContainerPool[i]->getTag() << " thread started with code: { " << threadContainerPool[i]->startThread() << " }");
                }
                INFO(getTag(), getDesc(), 
                    stringbuilder() << "Total Threads Started: { " << threadContainerPool.size() << " }");
                while (1) {
                    for (unsigned int i = 0; i < threadContainerPool.size(); i++) {
                        ...
                        if (statusCode > 0) {
                            DEBUG(getTag(), getDesc(), 
                                stringbuilder() << indexTag << " Exit Status Code: { " << statusCode << " }");
                            if (autoRestart == true) {
                                DEBUG(getTag(), getDesc(), 
                                    stringbuilder() << "Restarting Thread: { " << indexTag << " Auto-Restart-Flag: { " << autoRestart << " }");
                                int ret = threadContainerPool[i]->startThread();
                                if (ret != 0) {
                                    ERROR(getTag(), getDesc(), 
                                        stringbuilder() << indexTag << " Restart Return Code: { " << ret << " }");
                                }
                            }
                            else {
                                WARNING(getTag(), getDesc(), 
                                    stringbuilder() << indexTag << " Perhaps Turn On Auto-Restart-Flag: { " << autoRestart << " }");
                                WARNING(getTag(), getDesc(), 
                                    stringbuilder() << indexTag << " Setting Dead Thread Status Code");
